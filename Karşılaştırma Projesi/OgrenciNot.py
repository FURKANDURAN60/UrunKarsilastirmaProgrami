x = 0
urunSayisi = int(input('karşılaştırılacak ogrenci sayisini giriniz: '))
bilgisayarListesi = []
puanListesi = []
kriterSayisi = 4
kriterListesi = ['vize', 'quiz', 'odev', 'final',]
katsayiListesi = [0.3, 0.1, 0.1, 0.5]
for a in range(0, urunSayisi):
    bilgisayarListesi.append(input('{}. ogrenci ismi giriniz: '.format(a+1)))
while True:
    for d in range(0, urunSayisi):
        f = 0
        for e in range(0, kriterSayisi):
            print("{}'in  {} degerini giriniz: ".format(bilgisayarListesi[d], kriterListesi[e]))
            puan = float(input())
            f = f+(puan*katsayiListesi[e])
        puanListesi.append(f)
    break

tablo = zip(puanListesi, bilgisayarListesi)

siralanmis = sorted(tablo, key=lambda x: x[0], reverse=True)

for pair in siralanmis:
    print("{} urununun son puani {}".format(pair[1], pair[0]))
